...
GOBUILDPATH_REPLICATION_JOB=$(GOBUILDPATHINCONTAINER)/src/jobservice/job/impl/cmd
...
GOBUILDMAKEPATH_REPLICATION_JOB=$(GOBUILDMAKEPATH)/photon/replication_job
...
compile_replication_job:
	@echo "compiling binary for replication job (golang image)..."
	@$(DOCKERCMD) run --rm -v $(BUILDPATH):$(GOBUILDPATHINCONTAINER) -w $(GOBUILDPATH_REPLICATION_JOB) $(GOBUILDIMAGE) $(GOIMAGEBUILD_COMMON) -o $(GOBUILDPATHINCONTAINER)/$(GOBUILDMAKEPATH_REPLICATION_JOB)/$(REPLICATION_JOB)
	@echo "Done."
