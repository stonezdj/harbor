...
# Base images on Docker Hub only exist for amd64; disable pull on other architectures
...
GOBUILD=$(GOCMD) build
BUILDGOARCH := $(shell go env GOARCH)
GOCLEAN=$(GOCMD) clean
...
compile_core: lint_apis gen_apis
	...
	@$(DOCKERCMD) run --rm --platform linux/$(BUILDGOARCH) -e CGO_ENABLED=0 -v $(BUILDPATH):$(GOBUILDPATHINCONTAINER) -v $(GOMODCACHE):/root/go/pkg/mod  -w $(GOBUILDPATH_CORE) $(GOBUILDIMAGE) $(GOIMAGEBUILD_CORE) -o $(GOBUILDPATHINCONTAINER)/$(GOBUILDMAKEPATH_CORE)/$(CORE_BINARYNAME)
	@echo "Done."

compile_jobservice:
	@echo "compiling binary for jobservice (golang image)..."
	@$(DOCKERCMD) run --rm --platform linux/$(BUILDGOARCH) -e CGO_ENABLED=0 -v $(BUILDPATH):$(GOBUILDPATHINCONTAINER) -v $(GOMODCACHE):/root/go/pkg/mod  -w $(GOBUILDPATH_JOBSERVICE) $(GOBUILDIMAGE) $(GOIMAGEBUILD_COMMON) -o $(GOBUILDPATHINCONTAINER)/$(GOBUILDMAKEPATH_JOBSERVICE)/$(JOBSERVICEBINARYNAME)
	@echo "Done."

compile_registryctl:
	@echo "compiling binary for harbor registry controller (golang image)..."
	@$(DOCKERCMD) run --rm --platform linux/$(BUILDGOARCH) -e CGO_ENABLED=0 -v $(BUILDPATH):$(GOBUILDPATHINCONTAINER) -v $(GOMODCACHE):/root/go/pkg/mod  -w $(GOBUILDPATH_REGISTRYCTL) $(GOBUILDIMAGE) $(GOIMAGEBUILD_COMMON) -o $(GOBUILDPATHINCONTAINER)/$(GOBUILDMAKEPATH_REGISTRYCTL)/$(REGISTRYCTLBINARYNAME)
	@echo "Done."

compile_standalone_db_migrator:
	@echo "compiling binary for standalone db migrator (golang image)..."
	@$(DOCKERCMD) run --rm --platform linux/$(BUILDGOARCH) -e CGO_ENABLED=0 -v $(BUILDPATH):$(GOBUILDPATHINCONTAINER) -v $(GOMODCACHE):/root/go/pkg/mod  -w $(GOBUILDPATH_STANDALONE_DB_MIGRATOR) $(GOBUILDIMAGE) $(GOIMAGEBUILD_COMMON) -o $(GOBUILDPATHINCONTAINER)/$(GOBUILDMAKEPATH_STANDALONE_DB_MIGRATOR)/$(STANDALONE_DB_MIGRATOR_BINARYNAME)
	@echo "Done."
