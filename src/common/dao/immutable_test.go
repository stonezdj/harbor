...
func TestCreateImmutableRule(t *testing.T) {
	ir := &models.ImmutableRule{RepoFilter: "**", TagFilter: "**", ProjectID: 1}
	id, err := CreateImmutableRule(ir)
	if err != nil {
		t.Errorf("error: %+v", err)
	}
	if id <= 0 {
		t.Error("Can not create immutabe tag rule")
	}
...
func TestUpdateImmutableRule(t *testing.T) {
	...
	updatedIR := &models.ImmutableRule{ID: id, RepoFilter: "library", TagFilter: "1.2.0", ProjectID: 1}
	...
	if newIr.TagFilter != "1.2.0" || newIr.RepoFilter != "library" {
...
func TestGetImmutableRuleByProject(t *testing.T) {
	irs := []*models.ImmutableRule{
		{RepoFilter: "project1", TagFilter: "version1", ProjectID: 99},
		{RepoFilter: "project2", TagFilter: "version2", ProjectID: 99},
		{RepoFilter: "project3", TagFilter: "version3", ProjectID: 99},
		{RepoFilter: "project4", TagFilter: "version4", ProjectID: 99},
	}
	for _, ir := range irs {
