...
func (c *controller) SecuritySummary(ctx context.Context, projectID int64, withCVE bool, withArtifact bool) (*secHubModel.Summary, error) {
	sum, err := c.secHubMgr.Summary(ctx, c.defaultScannerUUID(ctx), projectID, nil)
	if err != nil {
		return nil, err
	}
	...
	sum.ScannedCnt, err = c.secHubMgr.ScannedArtifactsCount(ctx, c.defaultScannerUUID(ctx), projectID, nil)
	if err != nil {
		return nil, err
	}
	if withCVE {
		sum.DangerousCVEs, err = c.secHubMgr.DangerousCVEs(ctx, c.defaultScannerUUID(ctx), projectID, nil)
		if err != nil {
			return nil, err
		}
	}
	if withArtifact {
		sum.DangerousArtifacts, err = c.secHubMgr.DangerousArtifacts(ctx, c.defaultScannerUUID(ctx), projectID, nil)
		if err != nil {
			return nil, err
		}
	}
...
func (c *controller) scannedArtifactCount(ctx context.Context, projectID int64) (int64, error) {
	return c.secHubMgr.ScannedArtifactsCount(ctx, c.defaultScannerUUID(ctx), projectID, nil)
}
...
func (c *controller) defaultScannerUUID(ctx context.Context) string {
	reg, err := c.scannerMgr.GetDefault(ctx)
	if err != nil {
		log.Errorf("failed to get scanner, error %v", err)
		return ""
	}
	return reg.UUID
}
