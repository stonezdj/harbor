...
func (s *ScheduleTestSuite) TestCreateSchedule() {
	...
	p := purge.PurgeJobPolicy{}
	id, err := s.ctl.CreateSchedule(nil, purge.VendorType, "Daily", "* * * * * *", purge.SchedulerCallback, p, dataMap)
	s.Nil(err)
	s.Equal(int64(1), id)
}
...
func (s *ScheduleTestSuite) TestDeleteSchedule() {
	...
	s.Nil(s.ctl.DeleteSchedule(nil, purge.VendorType))
}
...
func (s *ScheduleTestSuite) TestGetSchedule() {
	...
	schedule, err := s.ctl.GetSchedule(nil, purge.VendorType)
	s.Nil(err)
