...
func (artifact *Artifact) SetAdditionLinkWithDigest(addition, version string, digest string) {
	if artifact.AdditionLinks == nil {
		artifact.AdditionLinks = make(map[string]*AdditionLink)
	}
	...
	// encode slash as %252F
	repo = repository.Encode(repo)
	href := fmt.Sprintf("/api/%s/projects/%s/repositories/%s/artifacts/%s/additions/%s", version, projectName, repo, digest, addition)
