...
func (o *ScanArtifactURL) Build() (*url.URL, error) {
	...
	var _path = "/projects/{project_name}/repositories/{repository_name}/artifacts/{reference}/scan"

	projectName := o.ProjectName
	if projectName != "" {
		_path = strings.Replace(_path, "{project_name}", projectName, -1)
	} else {
		return nil, errors.New("projectName is required on ScanArtifactURL")
	}
	...
	if reference != "" {
		...
		return nil, errors.New("reference is required on ScanArtifactURL")
	}
	...
	if repositoryName != "" {
		...
		return nil, errors.New("repositoryName is required on ScanArtifactURL")
	}
...
func (o *ScanArtifactURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ScanArtifactURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ScanArtifactURL")
	}
