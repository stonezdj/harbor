...
func (a *auditlogAPI) ListAuditLogEventTypes(ctx context.Context, params auditlog.ListAuditLogEventTypesParams) middleware.Responder {
	secCtx, ok := security.FromContext(ctx)
	if !ok {
		return a.SendError(ctx, errors.UnauthorizedError(errors.New("security context not found")))
	}
	...
	for _, v := range audit.EventIDList {
