...
func (p *purgeAPI) CreatePurgeSchedule(ctx context.Context, params purge.CreatePurgeScheduleParams) middleware.Responder {
	...
	// FIXME: the parameter can not passed correctly
...
func (p *purgeAPI) updateSchedule(ctx context.Context, cronType, cron string, policy pg.PurgeJobPolicy, extraParams map[string]interface{}) error {
	...
	if len(tasks) == 0 {
		return p.SendError(ctx,
			errors.New(nil).WithCode(errors.NotFoundCode).
				WithMessage("garbage collection %d log is not found", params.PurgeID))
	}
	log, err := p.taskCtl.GetTaskLog(ctx, pg.VendorType, tasks[0].ID)
	if err != nil {
		return p.SendError(ctx, err)
	}
	return operation.NewGetPurgeJobLogOK().WithPayload(string(log))
}
...
func (p *purgeAPI) createSchedule(ctx context.Context, cronType string, cron string, policy pg.PurgeJobPolicy, extraParam map[string]interface{}) error {
	if cron == "" {
