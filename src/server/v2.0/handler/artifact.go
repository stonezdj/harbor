...
func (a *artifactAPI) GetArtifact(ctx context.Context, params operation.GetArtifactParams) middleware.Responder {
	...
	err = assembler.NewVulAssembler(lib.BoolValue(params.WithScanOverview), lib.BoolValue(params.WithSbomOverview), parseScanReportMimeTypes(params.XAcceptVulnerabilities)).WithArtifacts(art).Assemble(ctx)
	if err != nil {
