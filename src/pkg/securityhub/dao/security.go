...
func applyVulFilter(ctx context.Context, sqlStr string, query *q.Query, params []interface{}) (queryStr string, newParam []interface{}) {
	if val, ok := query.Keywords["cve_id"]; ok {
		sqlStr += " and cve_id = ?"
		params = append(params, val)
	}
	if val, ok := query.Keywords["severity"]; ok {
		sqlStr += " and severity = ?"
		params = append(params, val)
	}
	if val, ok := query.Keywords["cvss_score_v3"]; ok {
		if r, ok := val.(*q.Range); ok {
			sqlStr += " and cvss_score_v3 between ? and ?"
	...
	if vaul, ok := query.Keywords["project_id"]; ok {
		sqlStr += " and project_id = ?"
		params = append(params, vaul)
	}
	if val, ok := query.Keywords["package"]; ok {
		sqlStr += " and vr.package = ?"
		params = append(params, val)
	}
...
func applyVulPagination(sqlStr string, query *q.Query) string {
	...
	return fmt.Sprintf("%v order by repository asc limit %d offset %d", sqlStr, query.PageSize, offSet)
}
...
func checkQFilter(query *q.Query, expectedTypeMap map[string]string) error {
	...
	for k := range query.Keywords {
		if typeName, exist := expectedTypeMap[k]; exist {
			switch typeName {
			case "range":
				if v, ok := query.Keywords[k].(*q.Range); !ok {
					return errors.BadRequestError(fmt.Errorf("keyword: %v, type %+v is not allowed", k, v))
				}
			case "string":
				if v, ok := query.Keywords[k].(string); !ok {
					return errors.BadRequestError(fmt.Errorf("keyword: %v, type %+v is not allowed", k, v))
				}
			case "int":
				if v, ok := query.Keywords[k].(int); !ok {
					return errors.BadRequestError(fmt.Errorf("keyword: %v, type %+v is not allowed", k, v))
				}
			}
		} else {
