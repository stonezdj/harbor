...
func (em *exportManager) Fetch(ctx context.Context, params Params) ([]Data, error) {
	...
	// The exported fields (package, version, CVE id, vendor attributes, ...) are
	// copied verbatim from scanner-adapter reports and are attacker-influenceable.
	// Neutralize any cell that a spreadsheet would evaluate as a formula before it
	// reaches the CSV writer (CWE-1236 / OWASP CSV injection).
