...
func (t *taskDAO) UpdateStatus(
	...
	if len(statusMessage) > 0 {
		msg = statusMessage[0]
		updateMsg = true
	} else if jobStatus == job.SuccessStatus || jobStatus == job.ErrorStatus {
		updateMsg = true
	}
	...
	var args []any
	if updateMsg {
		sql = `update task set status = ?, status_code = ?, status_revision = ?, status_message = ?, ` +
			`update_time = ?, end_time = ? ` +
			`where id = ? and (status_revision = ? and status_code < ? or status_revision < ?)`
		args = []any{
			status, statusCode, statusRevision, msg, now, endTime,
			id, statusRevision, statusCode, statusRevision,
		}
	} else {
		sql = `update task set status = ?, status_code = ?, status_revision = ?, ` +
			`update_time = ?, end_time = ? ` +
			`where id = ? and (status_revision = ? and status_code < ? or status_revision < ?)`
		args = []any{
			status, statusCode, statusRevision, now, endTime,
			id, statusRevision, statusCode, statusRevision,
		}
	}
