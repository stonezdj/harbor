...
type TaskDAO interface {
	...
	UpdateStatus(ctx context.Context, id int64, status string, statusRevision int64, statusMessage ...string) (err error)
...
func (t *taskDAO) UpdateStatus(ctx context.Context, id int64, status string, statusRevision int64, statusMessage ...string) error {
	ormer, err := orm.FromContext(ctx)
	if err != nil {
		return err
	}
	...
	// update run count and start time when status revision changes, and clear previous status_message
	sql := `update task set run_count = run_count +1, start_time = ?, status_message = '' 
	...
	if len(msg) > 0 {
		sql = `update task set status = ?, status_code = ?, status_revision = ?, status_message = ?, update_time = ?, end_time = ? 
			where id = ? and (status_revision = ? and status_code < ? or status_revision < ?) `
		_, err = ormer.Raw(sql, status, statusCode, statusRevision, msg, now, endTime,
			id, statusRevision, statusCode, statusRevision).Exec()
	} else if jobStatus == job.SuccessStatus {
		sql = `update task set status = ?, status_code = ?, status_revision = ?, status_message = '', update_time = ?, end_time = ? 
			where id = ? and (status_revision = ? and status_code < ? or status_revision < ?) `
