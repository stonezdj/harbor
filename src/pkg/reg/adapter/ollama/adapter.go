...
func init() {
	if err := adp.RegisterFactory(model.RegistryTypeOllama, new(factory)); err != nil {
		log.Errorf("failed to register factory for %s: %v", model.RegistryTypeOllama, err)
		return
	}
	log.Infof("the factory for adapter %s registered", model.RegistryTypeOllama)
}
	...
	info := &model.AdapterPattern{
		EndpointPattern: &model.EndpointPattern{
			...
			Endpoints: []*model.Endpoint{
				{
					Key:   "ollama",
					Value: "https://registry.ollama.ai",
				},
			},
		},
	}
...
func (a *Adapter) listArtifacts(repository string, filters []*model.Filter) ([]*model.Artifact, error) {
	...
	if a.registry.URL == "https://registry.ollama.ai" {
		// This is a special case for the Ollama registry.
		for _, t := range filters {
			if t.Type == model.FilterTypeTag {
				tagName := fmt.Sprintf("%v", t.Value)
				if _, ok := util.IsSpecificPath(tagName); ok {
					log.Infof("the pattern of tag filter is a specific tag name: %s", tagName)
...
func (a *Adapter) PingSimple() error {
	if a.registry.URL == "https://registry.ollama.ai" {
		// This is a special case for the Ollama registry, which is not a real registry.
		return nil
	}
	err := a.Ping()
	if err == nil {
