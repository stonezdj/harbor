...
func (suite *PurgeJobTestSuite) TestParseParams() {
	...
	j := &PurgeJob{}
	param := job.Parameters{"audit_retention_hour": 128, "dry_run": true}
	j.parseParams(param)
	...
	j2 := &PurgeJob{}
	param2 := job.Parameters{"audit_retention_hour": 24, "dry_run": false, "include_operations": "Delete,Create,Pull"}
...
func (suite *PurgeJobTestSuite) TestRun() {
	...
	j := &PurgeJob{auditMgr: auditManager}
	param := job.Parameters{"audit_retention_hour": 128, "dry_run": true}
	ret := j.Run(ctx, param)
	...
	j2 := &PurgeJob{auditMgr: auditManager}
	param2 := job.Parameters{"audit_retention_hour": 24, "dry_run": false}
