...
func (a *LoggerManager) Init(ctx context.Context, logEndpoint string) {
	...
	a.auditLogger = log.New(w, log.NewTextFormatter(), log.InfoLevel, 3)
	a.auditLogger.SetFallback(log.DefaultLogger())
...
// CheckEndpointActive check the liveness of the endpoint
